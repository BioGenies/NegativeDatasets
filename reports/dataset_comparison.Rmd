...
methods <- c("AMAP", "AmPEP", "AmpGram", "ampir-mature", "ampir-precursor", "AMPlify",
             "AMPScannerV2", "CSAMPPred", "dbAMP", "GabereNoble", "iAMP2L", "Wang", "Witten")

dataset_colors = c(Positive = "#ff4242", AMAP = "#f2b176", AmPEP = "#b4b4b4", AmpGram = "#f27676", `ampir-mature` = "#76bef2",
...
p <- read_fasta(paste0(data_path, "Training_method_AMAP_rep1.fasta"))
positive <- p[which(grepl("AMP=1", names(p)))]

df_neg <- get_prop_df(methods, 5, data_path)
df_pos <- calculate_properties(positive, "Positive", "1")
df_all <- bind_rows(mutate(df_pos, Dataset = "Positive"),
                    mutate(df_neg, Dataset = "Negative",
                           rep = as.character(rep))) %>% 
  mutate(method = factor(method, levels = c("Positive", methods)))
```
...
aa_comp_pos <- as.data.frame(table(aa_pos)/length(aa_pos)) %>% 
  setNames(c("aa", "Freq")) %>% 
        mutate(method = "Positive",
               rep = "1")
aa_comp_all <- bind_rows(mutate(aa_comp, 
                                rep = as.character(rep),
                                Dataset = "Negative"), 
                         mutate(aa_comp_pos,
                                Dataset = "Positive"))
...
ggbiplot(pca_res_all, choices = 1:2) +
  ...
  theme_bw() +
  geom_point(aes(color = pca_data_all[["method"]], shape = pca_data_all[["Dataset"]]), size = 3) +
...
ngram_counts_sum_all <- bind_rows(mutate(ngram_counts_sum, Dataset = "Negative"),
...
ggbiplot(pca_ngram_res, choices = 1:2, var.axes = FALSE) +
  ...
  geom_point(aes(color = ngram_counts_sum_all[["method"]], shape = ngram_counts_sum_all[["Dataset"]]), size = 3) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_color_manual("Sampling method", values = dataset_colors) +
  scale_shape_manual("Dataset", values = c(`Negative` = 16, `Positive` = 15))
...
aa_comp_peptides_pos <- lapply(names(positive), function(ith_prot) {
...
aa_comp_peptides_all <- bind_rows(aa_comp_peptides,
...
ggbiplot(pca_prop_res_all, choices = 1:2) +
  ...
  geom_point(aes(color = props_all[["method"]], shape = props_all[["dataset"]]), size = 3) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_color_manual("Sampling method", values = dataset_colors) +
  scale_shape_manual("Dataset", values = c(`Negative` = 16, `Positive` = 15))
