...
dataset_colors = c(AMAP = "#f2b176", AmPEP = "#b4b4b4", AmpGram = "#f27676", `ampir-mature` = "#76bef2",
                   `ampir-precursor` = "#76eff2", AMPlify = "#f2d676", AMPScannerV2 = "#b976f2", Wang = "#80f276",
                   CSAMPPred = "#76f2be", dbAMP = "#f276e8", GabereNoble = "#7688f2", iAMP2L = "#eff275", Witten = "#ccf276")


df <- calculate_properties(methods, 5, data_path)
```
...
```{r fig.height = 10}
aa_comp <- calculate_aa_comp_datasets(methods, 5, data_path)
...
## PCA on bigram and trigram counts

```{r  fig.height = 10}
ngram_counts_sum <- lapply(methods, function(i) {
  lapply(1:5, function(j) {
    ds <- read_fasta(paste0(data_path, "Training_method_", i, "_rep", j, ".fasta"))
    seqs <- ds[which(grepl("AMP=0", names(ds)))]
    
    count_multimers(seqs,
                    k_vector = c(2, 2, rep(3, 4)),
                    kmer_gaps_list = list(NULL, 1, c(0, 0), c(1, 0), c(0, 1), c(1, 1)),
...
ngram_counts_sum[is.na(ngram_counts_sum)] <- 0

pca_ngram_res <- prcomp(select(ngram_counts_sum, -c("method", "rep")), center = TRUE, scale = TRUE)

ggbiplot(pca_ngram_res, choices = 1:2, groups = ngram_counts_sum[["method"]], var.axes = FALSE) +
  ggtitle("PCA on bigram and trigram counts") +
  geom_point(aes(color = ngram_counts_sum[["method"]]), size = 3) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_color_manual("Sampling method", values = dataset_colors) 
...
aa_comp_peptides <- calculate_aa_comp_peptides(methods, 5, data_path)
...
combns <- combn(unique(aa_comp_peptides[["method"]]), 2, simplify = FALSE)
test_res_methods <- lapply(seq_along(combns), function(ith_combn) {
  test_dat <- filter(aa_comp_peptides, method  %in% combns[[ith_combn]])
  lapply(unique(aa_comp_peptides[["Amino acid"]]), function(ith_aa) {
