...
clustering_aa <- as.dendrogram(hclust(dist(t(as.matrix(aa_comp_heatmap_dat[,1:20])))))
...
aa_comp_clustered_dat[["Amino acid"]] <- factor(aa_comp_clustered_dat[["Amino acid"]],
                                                levels = colnames(aa_comp_heatmap_dat)[1:20][aa_order], ordered = TRUE)
...
```{r}
aa_comp_peptides <- lapply(1:12, function(i) {
  lapply(1:5, function(j) {
    ds <- read_fasta(paste0(data_path, "Training_method", i, "_rep", j, ".fasta"))
