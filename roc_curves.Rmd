...
for (i in seq(1, l, 24)) {
  if(i+23 <= l) {
    ...
    cat(paste0("\\caption{ROC curves ", i, "-", i+23, " of ", l, ". Each subplot presents results for five replications indicated by different line colors.}"))
    cat("\\end{figure}")
  } else {
    cat("\\begin{figure}")
    unlist(unlist(p, recursive = F), recursive = F)[i:l] %>% 
      gridExtra::grid.arrange(grobs = ., ncol = 4)
    cat(paste0("\\caption{ROC curves ", i, "-", l, " of ", l, ". Each subplot presents results for five replications indicated by different line colors.}"))
    cat("\\end{figure}")
  }
}
```
